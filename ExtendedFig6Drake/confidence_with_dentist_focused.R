...
for (i in sequence(1000)) {
	...
	sd_vector <- abs(runif(1, min=0.01, max=0.2)*params)
	if(runif(1) < 0.5) {
		sd_vector <- rep(0, length(params))
		lucky <- round(runif(1, min=4, max=11))
		sd_vector[lucky] <- runif(1, min=0.01, max=0.05)
		sd_vector[lucky-1] <- sd_vector[lucky]*runif(1, min=0.01, max=1)
		sd_vector[lucky+1] <- sd_vector[lucky]*runif(1, min=0.01, max=1)
	}
	...
	save(list=ls(), file="best_pdr_and_results_focal.rda")
