...
for (i in seq_along(subset_list)) {
	results[[i]] <- AdaptiveSupport(subset_list[[i]], tree=tree, delta=2, n_per_rep=5*parallel::detectCores(), n_per_good=10*parallel::detectCores(), cache_file=paste0("temp_delta2_", Sys.info()['nodename'], ".rda"))
	save(results, file=paste0("adaptive_delta2_", Sys.info()['nodename'], ".rda"))
}
