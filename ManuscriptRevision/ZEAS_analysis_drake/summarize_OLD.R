...
try(system("scp omearatc1.local:/share/ZEAS_analysis_drake/omearatc1_results.rda ."))
try(system("scp omearatc1.local:/share/ZEAS_analysis_drake/omearatc2_results.rda ."))

all_results_model_average <- data.frame()
all_results_best <- data.frame()
all_results_overview <- data.frame()

inputs <- c("omearatc1_results.rda", "omearatc2_results.rda")
