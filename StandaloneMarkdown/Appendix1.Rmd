...
knitr::opts_chunk$set(echo = FALSE, warning=FALSE, cache=TRUE, fig.width=7, fig.height=7)
set.seed(1859)
```
...
```{r backboneplotdetail, fig.height=2}
par(mfrow=c(1,2))
...
bd.results[2, "balanced"] <- ape::birthdeath(bb1)$dev
bd.results[2, "pectinate"] <- ape::birthdeath(bb2)$dev
...
# age_grid  = seq(from=0, to=max(ape::branching.times(bb1)), by=0.01) # choose a sufficiently fine age grid
# lambda    = 1*exp(beta*age_grid) # define lambda on the age grid
# mu        = 0.2*lambda # assume similarly shaped but smaller mu
# 
# 
# bd.results[3, "balanced"] <- castor::loglikelihood_hbd( bb1, 
...
bd.results[5, "balanced"] <- hisse::MiSSE(bb1, turnover=c(1,2), eps=c(1,2))$loglik
bd.results[5, "pectinate"] <- hisse::MiSSE(bb2, turnover=c(1,2), eps=c(1,2))$loglik
